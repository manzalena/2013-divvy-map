...
class Vehicle {
  ...
  void follow(tripPath p) {
    ...
    PVector normal = null;
    PVector target = null;
    float worldRecord = 1000000000;  // Start with a very high record distance that can easily be beaten
  ...
  void display() {
    ...
    color stop = color(255,0,50);
    color start = color(0,191,255);
    float locale = map(location.x,0,width,0,1);
    color inter = lerpColor(stop, start, locale);
    fill(inter);
//    fill(0,191,255);
    //tint(255,10);
